 ...
 * 読んだ内容を、扱える形にする。
 *
 * **平文で置く。** 同じ PII は元のファイルにもタスクの履歴にも平文なので、対応表だけ暗号化しても
 * 守りは上がらない。壊れた・版が違うときは、誤った値ではなく失敗を出す。
 */
...
export class FileMappingStore {
	...
	private async write(catalog: Catalog, _existed: boolean): Promise<void> {
	...
	inspect(identity: string): Promise<FileMappingRecord | undefined> {
	...
	enable(identity: string, entries: readonly FileMappingEntry[] = []): Promise<FileMappingRecord> {
		return this.exclusive(async () => {
			const { catalog, exists } = await this.read()
	...
	appendIfEnabled(identity: string, entries: readonly FileMappingEntry[]): Promise<boolean> {
		return this.exclusive(async () => {
			const { catalog, exists } = await this.read()
			const record = catalog.files[identity]
