...
| 出来るもの | 対象 | 固有名詞検出 |
| --- | --- | --- |
| `copilot-pii-guard-linux-x64-0.2.6.vsix` | Linux (x64) | 動く |
| `copilot-pii-guard-win32-x64-0.2.6.vsix` | Windows (x64) | 動く |
| `copilot-pii-guard-0.2.6.vsix` | それ以外 | 動かない（基本検出は動く） |

## 入れ方

```sh
code --install-extension bin/copilot-pii-guard-linux-x64-0.2.6.vsix
```
...
```sh
unzip copilot-pii-guard-linux-x64-0.2.6.vsix -d pii-guard-model-builder
...
```powershell
Copy-Item .\copilot-pii-guard-win32-x64-0.2.6.vsix .\copilot-pii-guard.zip
