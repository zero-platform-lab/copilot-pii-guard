...
| このファイルのFile Vaultを有効にする | このファイルの対応表をセッション間で保持する |
| このファイルのFile Vaultを消去する | 対応表を消去し、永続化を無効にする |
...
File Vaultはファイルごとの明示的なオプトインです。対応表を平文のJSONでワークスペース固有の拡張機能専用領域へ保存します。保管ディレクトリに`.gitignore`を置いてリポジトリへコミットさせません。
...
| `piiGuard.fileVault.maxBytes` | `5242880` | File Vault全体のデータ量の上限（バイト）。`0` は無制限 |
