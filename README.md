...
| 出来るもの | 対象 | 固有名詞検出 |
| --- | --- | --- |
| `copilot-pii-guard-linux-x64-0.2.9.vsix` | Linux (x64) | 動く |
| `copilot-pii-guard-win32-x64-0.2.9.vsix` | Windows (x64) | 動く |
| `copilot-pii-guard-0.2.9.vsix` | それ以外 | 動かない（基本検出は動く） |

## 入れ方

```sh
code --install-extension bin/copilot-pii-guard-linux-x64-0.2.9.vsix
```
...
右クリックの **PII Guard** メニューに、伏せ字化、辞書、ファイル対応表の操作を追加します。
...
| このファイルのファイル対応表を有効にする | このファイルの対応表をセッション間で保持する |
| このファイルのファイル対応表を消去する | 対応表を消去し、永続化を無効にする |
| このファイルのファイル対応表状態を確かめる | 有効・無効と保存している対応数を表示する |

`Ctrl+Shift+P` からは、さらにファイル対応表の一括消去を含む操作ができる。

| 項目 | 何をするか |
| --- | --- |
| PII Guard: セッション対応表を消去する | 現在のセッションで保持している伏せ字対応を、確認後にすべて消去する |
| PII Guard: 選んだファイル対応表を消去する | 一覧から選んだ複数ファイルの対応を確認後に消去する |
| PII Guard: すべてのファイル対応表を消去する | ワークスペースの全ファイル対応表を確認後に消去する |
...
セッション対応表は、元の値と伏せ字の対応を拡張機能ホストのメモリ内だけに保持し、`@mask`の
会話とエディタ操作で共有します。VS Codeの再読み込みまたは終了で破棄されます。コマンドで
消去した対応は復元できないため、消去前に対応数を表示して確認します。

ファイル対応表はファイルごとの明示的なオプトインです。対応表を平文のJSONでワークスペース固有の拡張機能専用領域へ保存します。保管ディレクトリに`.gitignore`を置いてリポジトリへコミットさせません。保管ルートは設定`piiGuard.fileMapping.root`で変えられます（絶対パス、既定は拡張機能専用領域）。ワークスペース内を指すと、対応表を読み取ってLLMへ送る恐れがあるため警告します。
...
| `piiGuard.sessionMapping.maxEntries` | `10000` | セッション対応表へ登録できる対応数の上限。`0` は無制限 |
| `piiGuard.fileMapping.retentionDays` | `30` | ファイル対応表を最終利用から保持する日数。`0` は時間による期限なし |
| `piiGuard.fileMapping.maxFiles` | `100` | ファイル対応表を有効にできるファイル数の上限。`0` は無制限 |
| `piiGuard.fileMapping.maxEntriesPerFile` | `1000` | 1ファイルに保存できる対応数の上限。`0` は無制限 |
| `piiGuard.fileMapping.maxBytes` | `5242880` | ファイル対応表全体のデータ量の上限（バイト）。`0` は無制限 |
| `piiGuard.fileMapping.root` | 空 | 対応表の保管ルート（絶対パス）。空なら拡張機能専用領域。設定するとルート配下にワークスペースごとの区画を作る。ワークスペース内は避ける（読み取られLLMへ送られる恐れ） |
...
```sh
unzip copilot-pii-guard-linux-x64-0.2.9.vsix -d pii-guard-model-builder
...
```powershell
Copy-Item .\copilot-pii-guard-win32-x64-0.2.9.vsix .\copilot-pii-guard.zip
